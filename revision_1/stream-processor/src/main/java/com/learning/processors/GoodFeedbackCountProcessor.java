...
//@Configuration
public class GoodFeedbackCountProcessor {
	...
	KStream<String, String> badWordsStream = splittedStreams[1];
		...
		//count of all unique good words
		//size of ktable will be each word count as word is unique already
		KStream<String, String> goodWordsCountStream = goodWordsStream.groupByKey().count().toStream()
						.mapValues(String::valueOf);
		goodWordsCountStream.to(OUTPUT_TOPIC_COUNT,Produced.with(stringSerde, stringSerde));
		
		//bad word count
		KStream<String, String> badWordsCountStream =
				badWordsStream.groupByKey().count().toStream().mapValues(String::valueOf);
		badWordsCountStream.to(OUTPUT_BAD_TOPIC_COUNT, Produced.with(stringSerde, stringSerde));
